...
// RN's WebSocket can't set custom headers at connect time, so the JWT is
// passed as a query param instead of the Authorization header used by REST.
export async function chatWsUrl(matchId: string): Promise<string> {
  const t = (await token()) ?? "";
  const wsBase = BASE.replace(/^http/, "ws");
  return `${wsBase}/api/ws/chat/${matchId}?token=${encodeURIComponent(t)}`;
}
...
export const api = {
  ...
  swipe: (target_id: string, direction: "like" | "pass") =>
    request<{ ok: boolean; match: any; paywalled?: boolean }>("/swipes", {
      method: "POST",
  ...
  suggestLocations: (matchId: string) => request<any[]>(`/matches/${matchId}/suggest-locations`),

  billingStatus: () => request<any>("/billing/status"),
  createCheckout: () => request<{ checkout_url: string }>("/billing/create-checkout", { method: "POST" }),
