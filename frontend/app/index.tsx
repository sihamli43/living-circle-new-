...
// C.cyan (Gold #FFC107) is too light to read as plain text on a white
// background — this deepened variant is used wherever Gold needs to be
// legible body/label text rather than an icon fill or a badge on Navy.
const GOLD_TEXT = "#8A6200";
    ...
    <View
      ...
      style={[
        StyleSheet.absoluteFill,
        {
          ...
          ...(({
            backgroundImage:
              "linear-gradient(rgba(255,82,82,0.25) 1px, transparent 1px), linear-gradient(90deg,rgba(255,82,82,0.25) 1px,transparent 1px)",
...
export default function LandingPage() {
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      {/* ── NAVY HERO BAND (nav + hero only — rest of the page is white) ── */}
      <View style={styles.heroSection}>
        <GridOverlay />

        {/* Ambient blobs */}
        <GlowBlob color="#17A2B8" top={-120} left={-80}  size={380} opacity={0.10} />
        <GlowBlob color="#FF5252" top={300}  right={-100} size={320} opacity={0.09} />
        <GlowBlob color="#FFC107" top="40%"  left="30%"  size={260} opacity={0.06} />
        <GlowBlob color="#17A2B8" bottom={200} right={60} size={200} opacity={0.08} />
      ...
      <View style={styles.statsBar}>
        <Stat value="500+" label="Roommates" color={GOLD_TEXT} />
        <View style={styles.statsDivider} />
        <Stat value="50+"  label="Localities" color={C.brand} />
        <View style={styles.statsDivider} />
        <Stat value="94%"  label="Match Rate" color={C.coral} />
        <View style={styles.statsDivider} />
        <Stat value="100%" label="Bangalore" color={C.brand} />
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.featureGrid, isWide && { flexDirection: "row", flexWrap: "wrap" }]}>
          {[
            { icon: "shield-checkmark-outline", title: "Privacy First",        body: "Room photos hidden until you match. Your number is never shared.",               color: C.brand },
            { icon: "heart-outline",            title: "Smart Matching",        body: "Algorithm scores lifestyle, budget, locality & language compatibility.",          color: C.coral },
            { icon: "location-outline",         title: "Neighbourhood Map",     body: "See metro, gym, ATM & restaurant distances post-match. No surprises.",           color: GOLD_TEXT },
            { icon: "shield-outline",           title: "ID Verification",       body: "Verified badge for users who submit a government ID. +40% more matches.",        color: C.brand },
            { icon: "chatbubble-outline",       title: "In-App Chat",           body: "Safe, encrypted messaging — no need to share personal numbers.",                 color: C.coral },
            { icon: "people-outline",           title: "Bangalore Locals Only", body: "Only real Bangalore profiles — no bots, no out-of-city fakes.",                  color: GOLD_TEXT },
          ].map((f) => (
      ...
      <View style={[styles.safetySection]}>
        ...
        <View style={styles.safetyRight}>
          {[
            { icon: "lock-closed-outline",      label: "End-to-end encrypted chat",              color: C.brand },
            { icon: "eye-off-outline",           label: "Photos hidden until mutual match",        color: GOLD_TEXT },
            { icon: "shield-checkmark-outline",  label: "ID verification system",                  color: C.brand },
            { icon: "flag-outline",              label: "1-tap report & block",                    color: C.coral },
            { icon: "warning-outline",           label: "Safety briefing on every new match",      color: GOLD_TEXT },
            { icon: "call-outline",              label: "Emergency helpline numbers built in",     color: C.coral },
      ...
      <View style={styles.finalCTA}>
        <GlowBlob color="#FF5252" top={-60} right={40} size={200} opacity={0.1} />
        <GlowBlob color="#17A2B8" bottom={-40} left={20} size={180} opacity={0.1} />
...
const styles = StyleSheet.create({
  ...
  // Navy hero band (nav + hero live inside this; everything else is white)
  heroSection: { backgroundColor: "#001F3F" },

  // Nav
  nav: {
    flexDirection: "row", alignItems: "center", justifyContent: "space-between",
    paddingHorizontal: isWide ? 48 : S.xl, paddingVertical: 16,
    borderBottomWidth: 1, borderBottomColor: "rgba(255,82,82,0.15)",
  ...
  navLoginText: { color: "rgba(255,255,255,0.85)", fontSize: 14, fontWeight: "600" },
  navSignupBtn: {
    paddingHorizontal: 18, paddingVertical: 9, borderRadius: R.pill,
    backgroundColor: C.coral,
    ...(Platform.OS === "web" ? ({ boxShadow: "0 4px 20px rgba(255,82,82,0.5)" } as any) : {}),
  },
  ...
  heroBadge: {
    ...
    borderRadius: R.pill, borderWidth: 1, borderColor: "rgba(22,163,74,0.5)",
    backgroundColor: "rgba(22,163,74,0.15)",
  },
  heroBadgeDot: {
    width: 7, height: 7, borderRadius: 4, backgroundColor: "#4ADE80",
    ...(Platform.OS === "web" ? ({ boxShadow: "0 0 8px #4ADE80" } as any) : {}),
  },
  ...
  heroAccentCyan: {
    color: "#17A2B8",
    ...(Platform.OS === "web" ? ({ textShadow: "0 2px 20px rgba(255,82,82,0.35)" } as any) : {}),
  },
  heroSub: {
    fontSize: isWide ? 18 : 15, color: "rgba(255,255,255,0.82)",
  ...
  ctaPrimary: {
    ...
    ...(Platform.OS === "web" ? ({ boxShadow: "0 4px 24px rgba(255,82,82,0.55), 0 2px 12px rgba(0,0,0,0.35)" } as any) : {
  ...
  heroDisclaimer: { color: "rgba(255,255,255,0.55)", fontSize: 12, letterSpacing: 0.3 },

  // Stats
  statsBar: {
    flexDirection: "row", justifyContent: "center", alignItems: "center",
    paddingVertical: 28, paddingHorizontal: S.xl,
    borderTopWidth: 1, borderBottomWidth: 1, borderColor: "rgba(255,82,82,0.12)",
    backgroundColor: "rgba(255,82,82,0.03)", gap: 0,
    flexWrap: "wrap",
  },
  ...
  statsDivider: { width: 1, height: 36, backgroundColor: "rgba(0,31,63,0.1)" },

  // Divider
  divider: { flexDirection: "row", alignItems: "center", paddingHorizontal: isWide ? 80 : S.xl, marginVertical: 4 },
  dividerLine: { flex: 1, height: 1, backgroundColor: "rgba(255,82,82,0.15)" },
  dividerDot: {
    width: 6, height: 6, borderRadius: 3, backgroundColor: C.cyan, marginHorizontal: 12,
    ...(Platform.OS === "web" ? ({ boxShadow: "0 0 8px #FFC107" } as any) : {}),
  },

  // Section
  section: { paddingHorizontal: isWide ? 80 : S.xl, paddingVertical: 60 },
  sectionLabel: { color: GOLD_TEXT, fontSize: 11, fontWeight: "900", letterSpacing: 2.5, marginBottom: 14 },
  sectionTitle: {
    fontSize: isWide ? 38 : 28, fontWeight: "900", color: C.onSurface,
  ...
  featureGrid: { gap: 14 },
  featureCard: {
    backgroundColor: C.surfaceSecondary, borderRadius: R.lg, borderWidth: 1,
    padding: 22, gap: 12,
    ...(isWide ? { width: CARD_W } : {}),
  ...
  featureTitle: { fontSize: 16, fontWeight: "800", color: C.onSurface },
  ...
  stepTitle: { fontSize: 17, fontWeight: "800", color: C.onSurface, marginBottom: 6 },
  stepBody: { fontSize: 14, color: C.onSurfaceSecondary, lineHeight: 22 },
  stepConnector: {
    width: 2, height: 36, backgroundColor: C.border,
  ...
  finalCTABtn: {
    ...
    borderRadius: R.pill,
    ...(Platform.OS === "web" ? ({ boxShadow: "0 6px 28px rgba(255,82,82,0.55)" } as any) : {
  ...
  footerBrandText: { color: C.onSurface, fontSize: 16, fontWeight: "900", letterSpacing: 1 },
  footerTagline: { color: C.onSurfaceTertiary, fontSize: 12 },
  footerLinks: { flexDirection: "row", alignItems: "center", gap: 12 },
  footerLink: { color: GOLD_TEXT, fontSize: 13, fontWeight: "600" },
