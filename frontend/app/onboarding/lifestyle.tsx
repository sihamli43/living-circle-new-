...
export default function Lifestyle() {
  ...
  // Guards the final question's save+navigate round-trip (two sequential API
  // calls) from being re-triggered by a repeat tap before it resolves — without
  // this, an impatient tap during network latency looks like the screen is
  // "stuck repeating" on the last question.
  ...
  const advance = async (answer?: string) => {
    ...
    if (!isLast) {
      ...
      setSubmitting(true);
      try {
        await api.updateMe({ lifestyle: next, onboarded: true });
  ...
  return (
    <Screen
      ...
      onBack={onBack}
      cta={submitting ? "Saving…" : isLast ? "Almost done" : "Next"}
      ctaDisabled={submitting || (!q.skippable && !selected)}
      onCta={onNext}
    >
