...
function buildMapHtml(data: LocationData, activeAmenity: Amenity | null): string {
  const amenityMarkersJs = data.amenities.slice(0, 24).map((a) => {
    const cfg = AMENITY_CFG[a.type] ?? { emoji: "📍", color: "#17A2B8" };
  ...
  const routeLine = data.my_lat && data.my_lng
    ? `L.polyline([[${data.my_lat},${data.my_lng}],[${data.match_lat},${data.match_lng}]],{color:'#17A2B8',weight:2.5,opacity:0.85,dashArray:'9,6'}).addTo(map);`
    : "";

  const activeRouteLine = activeAmenity
    ? `L.polyline([[${data.match_lat},${data.match_lng}],[${activeAmenity.lat},${activeAmenity.lng}]],{color:'#FF5252',weight:2.5,opacity:0.9,dashArray:'6,4'}).addTo(map);`
    : "";

  const myMarker = data.my_lat && data.my_lng
    ? `L.marker([${data.my_lat},${data.my_lng}],{icon:L.divIcon({html:'<div style="width:42px;height:42px;border-radius:50%;background:#17A2B8;display:flex;align-items:center;justify-content:center;font-size:20px;box-shadow:0 0 20px #17A2B8BB,0 0 40px #17A2B844;border:2px solid #fff;">🏠</div>',iconSize:[42,42],iconAnchor:[21,21],className:''})}).addTo(map).bindPopup('<b>Your Area</b><br>${(data.my_locality ?? "").replace(/'/g, "\\'")}');`
    : "";
...
<style>
*{margin:0;padding:0;box-sizing:border-box}body{background:#FFFFFF}
#map{width:100%;height:100vh}
.leaflet-control-attribution{display:none!important}
.leaflet-popup-content-wrapper{background:#FFFFFF;color:#001F3F;border:1px solid rgba(23,162,184,.5);border-radius:14px;box-shadow:0 0 20px rgba(23,162,184,.25)}
.leaflet-popup-tip{background:#FFFFFF}
.leaflet-popup-content{color:#4A5C6E;font-family:system-ui,sans-serif;font-size:13px;line-height:1.5}
.leaflet-popup-content b{color:#17A2B8;font-size:14px}
...
L.marker([${data.match_lat},${data.match_lng}],{icon:L.divIcon({
  html:'<div style="width:46px;height:46px;border-radius:50%;background:#FF5252;display:flex;align-items:center;justify-content:center;font-size:22px;box-shadow:0 0 22px #FF5252BB,0 0 44px #FF525244;border:2px solid #fff;">📍</div>',
    ...
    <View style={[styles.distCard, { borderColor: color + "70", shadowColor: color }]}>
      ...
      <View style={{ flex: 1, gap: 6 }}>
        ...
        <View style={{ flexDirection: "row", gap: 6, flexWrap: "wrap" }}>
          {driveMin   != null && <ModePill emoji="🚗" label="Drive"   min={driveMin}   color="#17A2B8" />}
        ...
        <View style={styles.amenityExpanded}>
          ...
          <View style={styles.modeGrid}>
            <View style={styles.modeCard}>
              ...
              <Text style={[styles.modeCardMin, { color: "#17A2B8" }]}>{item.drive_min} min</Text>
            </View>
            <View style={styles.modeCard}>
              <Text style={styles.modeCardEmoji}>🚌</Text>
...
const styles = StyleSheet.create({
  ...
  requestBtn: {
    ...
    borderWidth: 1.5, borderColor: C.coral, backgroundColor: "rgba(255,82,82,0.1)",
    shadowColor: C.coral, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.5, shadowRadius: 8, elevation: 6,
  },
  requestBtnDone: { borderColor: C.success, backgroundColor: "rgba(22,163,74,0.1)", shadowColor: C.success },
  ...
  distBadge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: R.md, borderWidth: 1, backgroundColor: "rgba(0,31,63,0.06)" },
  distBadgeText: { fontSize: 13, fontWeight: "800" },

  // Expanded detail
  amenityExpanded: { padding: 14, borderTopWidth: 1, borderTopColor: C.border, backgroundColor: "rgba(0,31,63,0.03)", gap: 10 },
  farWarning: { backgroundColor: "rgba(255,193,7,0.12)", borderWidth: 1, borderColor: "rgba(255,193,7,0.5)", borderRadius: R.md, padding: 10 },
  farWarningText: { color: "#8A6200", fontSize: 12, fontWeight: "600" },
